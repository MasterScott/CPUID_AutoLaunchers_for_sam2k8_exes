...
#AutoIt3Wrapper_Outfile=F1_2015_AutoLauncher.exe
#AutoIt3Wrapper_Compression=4
#AutoIt3Wrapper_UPX_Parameters=-9 --strip-relocs=0 --compress-exports=0 --compress-icons=0
#AutoIt3Wrapper_Res_Description=F1 2015 Launcher
...
#pragma compile(FileDescription, 'F1 2015 Launcher')
#pragma compile(FileVersion, 1.0.0.47)
#pragma compile(InternalName, 'F1 2015 Launcher')
#pragma compile(LegalCopyright, '2014, SalFisher47')
#pragma compile(OriginalFilename, F1_2015_AutoLauncher.exe)
#pragma compile(ProductName, 'F1 2015 Launcher')
#pragma compile(ProductVersion, 1.0.0.47)
#EndRegion ;**** Pragma Compile ****
; === UniCrack Installer.au3 =======================================================================================================
; Title .........: F1 2015 Launcher
; Version .......: 1.0.0.47
; AutoIt Version : 3.3.10.2
; Language ......: English
; Description ...: F1 2015 Launcher
...
FileInstall("F1_2015_AutoLauncher\RequiredSoftware\7za.exe", @AppDataCommonDir & "\SalFisher47\7za\7za.exe", 0)
FileInstall("F1_2015_AutoLauncher\RequiredSoftware\7-Zip.chm", @AppDataCommonDir & "\SalFisher47\7za\7-Zip.chm", 0)
FileInstall("F1_2015_AutoLauncher\RequiredSoftware\License.txt", @AppDataCommonDir & "\SalFisher47\7za\License.txt", 0)
FileInstall("F1_2015_AutoLauncher\RequiredSoftware\Readme.txt", @AppDataCommonDir & "\SalFisher47\7za\Readme.txt", 0)

If Not FileExists(@AppDataCommonDir & "\SalFisher47\xd3") Then DirCreate(@AppDataCommonDir & "\SalFisher47\xd3")
FileInstall("F1_2015_AutoLauncher\RequiredSoftware\xd3.exe", @AppDataCommonDir & "\SalFisher47\xd3\xd3.exe", 0)

FileInstall("F1_2015_AutoLauncher.ini", @ScriptDir & "\F1_2015_AutoLauncher.ini", 0)

$7z_amd = @ScriptDir & "\F1_2015_AutoLauncher\Bin_AMD.7z"
$7z_intel = @ScriptDir & "\F1_2015_AutoLauncher\Bin_Intel.7z"
$7z_backup = @ScriptDir & "\F1_2015_AutoLauncher\Bin_Backup.7z"

$ini_cpu_detected = IniRead(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_detected", "")
$ini_cpu_forced = IniRead(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_forced", "")
$ini_cpu_forced_orig = $ini_cpu_forced
$ini_cpu_patched = IniRead(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_patched", "")
$ini_msg = IniRead(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Optional", "msg", "")
...
; cpu_forced <> cpu_patched (exes HAVEN'T BEEN patched for cpu_forced - have to be patched now)
	...
	; detected cpu is AMD
		...
		If $ini_cpu_detected <> $reg_cpu Then
			IniWrite(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_detected", " " & $reg_cpu)
			$ini_cpu_detected = $reg_cpu
			$ini_cpu_forced = $ini_cpu_detected
		EndIf
		$cpu_count = _FileCountLines(@ScriptDir & "\F1_2015_AutoLauncher\Bin_AMD.txt")
		$hFile = FileOpen(@ScriptDir & "\F1_2015_AutoLauncher\Bin_AMD.txt")
		...
		; exes CAN BE patched for this cpu
			IniWrite(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_forced", " " & $ini_cpu_forced)
			; wrote cpu_forced to F1_2015_AutoLauncher.ini
			...
			If Not FileExists($7z_amd) Then
				MsgBox(64, "F1_2015_AutoLauncher", "..\F1_2015_AutoLauncher\Bin_AMD.7z NOT FOUND!")
			Else
				$7z_unpack_line = ' x -y -o' & '"' & @TempDir & '\TTP_xdelta' & '"' & ' ' & '"' & $7z_amd & '"' & ' ' & '"' & $ini_cpu_forced & '\*' & '"'
				ShellExecuteWait("7za.exe", $7z_unpack_line, @AppDataCommonDir & "\SalFisher47\7za", "", @SW_HIDE)
				$xdelta_patch_line_talos = ' -d -s ' & '"' & @ScriptDir & '\F1_2015.exe' & '"' & ' ' & '"' & @TempDir & '\TTP_xdelta\' & $ini_cpu_forced & '\F1_2015 - ' & $ini_cpu_forced & '.xd3' & '"' & ' ' & '"' & @TempDir & '\TTP_xdelta\' & $ini_cpu_forced & '\F1_2015.exe' & '"'
				RunWait(@AppDataCommonDir & "\SalFisher47\xd3\xd3.exe" & $xdelta_patch_line_talos, @AppDataCommonDir & "\SalFisher47\xd3", @SW_HIDE)
				FileCopy(@TempDir & '\TTP_xdelta\' & $ini_cpu_forced & '\F1_2015.exe', @ScriptDir & '\F1_2015.exe', 1)
				DirRemove(@TempDir & '\TTP_xdelta', 1)
				IniWrite(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_patched", " " & $ini_cpu_forced)
			EndIf
			; patched exes
			If StringInStr($ini_cpu_forced, $ini_cpu_detected) Then
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "Executable PATCHED for your CPU: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game should run properly..." & @CRLF & "Play F1 2015 now?")
			Else
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "Executable PATCHED for this CPU: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game might not run properly..." & @CRLF & "Play F1 2015 now?")
			EndIf
			If $MsgBox == 6 Then
				ShellExecute(@ScriptDir & "\F1_2015.exe", $CmdLineRaw, @ScriptDir)
				Exit
		...
		; exes CAN'T BE patched for this cpu
			IniWrite(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_forced", " AMD FX 8350")
			; wrote cpu_forced to F1_2015_AutoLauncher.ini
			$7z_unpack_line = ' x -y -o' & '"' & @ScriptDir & '"' & ' ' & '"' & $7z_backup & '"' & ' ' & '"' & '*' & '"'
			ShellExecuteWait("7za.exe", $7z_unpack_line, @AppDataCommonDir & "\SalFisher47\7za", "", @SW_HIDE)
			; unpacked exes to be patched by xdelta3
			...
			Else
				...
				FileCopy(@TempDir & '\TTP_xdelta\' & $ini_cpu_forced & '\F1_2015.exe', @ScriptDir & '\F1_2015.exe', 1)
				DirRemove(@TempDir & '\TTP_xdelta', 1)
				IniWrite(@ScriptDir & "\F1_2015_AutoLauncher.ini", "Settings", "cpu_patched", " AMD FX 8350")
			EndIf
			; patched exes
			If StringInStr($ini_cpu_forced, $ini_cpu_detected) Then
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "It seems your CPU is not supported: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"Executable PATCHED for AMD FX 8350..." & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game won't run properly..." & @CRLF & "Play F1 2015 now?")
			Else
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "It seems this CPU is not supported: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"Executable PATCHED for AMD FX 8350..." & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game won't run properly..." & @CRLF & "Play F1 2015 now?")
			EndIf
	...
	; detected cpu is Intel
		...
		If $ini_cpu_detected <> $reg_cpu Then
			...
			$ini_cpu_forced = $ini_cpu_detected
		EndIf
		$cpu_count = _FileCountLines(@ScriptDir & "\F1_2015_AutoLauncher\Bin_Intel.txt")
		$hFile = FileOpen(@ScriptDir & "\F1_2015_AutoLauncher\Bin_Intel.txt")
		For $i = 1 to $cpu_count
		...
		; exes CAN BE patched for this cpu
			...
			If Not FileExists($7z_intel) Then
				MsgBox(64, "F1_2015_AutoLauncher", "..\F1_2015_AutoLauncher\Bin_Intel.7z NOT FOUND!")
			Else
...
; cpu_forced == cpu_patched (exes HAVE BEEN patched for cpu_forced - no need to be patched again)
	...
	; detected cpu is AMD
		...
		If $ini_msg == 1 Then
			If StringInStr($ini_cpu_forced, $ini_cpu_detected) Then
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "Executable COMPATIBLE with your CPU: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game should run properly..." & @CRLF & "Play F1 2015 now?")
			Else
				$MsgBox = MsgBox(68, "F1_2015_AutoLauncher", "Executable COMPATIBLE with this CPU: " & @CRLF & "---> " & $ini_cpu_forced_orig & @CRLF & @CRLF & _
								"To patch it for other CPUs, edit cpu_forced in F1_2015_AutoLauncher.ini, then run this again..." & @CRLF & @CRLF & _
								"The game might not run properly..." & @CRLF & "Play F1 2015 now?")
			EndIf
			If $MsgBox == 6 Then
